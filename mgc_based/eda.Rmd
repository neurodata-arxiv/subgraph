...
df = data.frame("total_edges"= c(total_edges), "id"= as.factor((c(1:m))),"genotype"= as.factor((GENOTYPEPick)))
...
Use MLE for p_1 and p_2 in the two binomial population, plug in n^2*p(1-p) as the two population variance, compute variance, do z-test (as CLT approximation for Binomial)
...
zstat = z.test2sam(x1,x2, sigma2.x1,sigma2.x2)
...
Use MLE for p_1 and p_2 in the two binomial population, plug in n^2/4*p(1-p) as the two population variance, compute variance, do z-test (as CLT approximation for Binomial)
